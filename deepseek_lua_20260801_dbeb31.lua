-- ==================================================
-- REDDIEHUB v23.1 - ULTIMATE PREMIUM EDITION
-- 2-Button Teleport System - Rainbow Style
-- Anti-Cheat Bypass - Always Visible Choose
-- ==================================================

-- Safety pcall for all operations
local success, err = pcall(function()

print("Loading ReddieHub v23.1...")
...
-- Bypass remote event checks
local function bypassAntiCheat()
    pcall(function()
        -- Disable any anti-cheat scripts
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Script") or obj:IsA("LocalScript") then
                if obj.Name:lower():find("anticheat") or obj.Name:lower():find("anti") or obj.Name:lower():find("cheat") then
                    pcall(function()
                        obj.Disabled = true
                    end)
                end
        ...
        -- Disable remote event checks
        for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
            if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                if obj.Name:lower():find("anti") or obj.Name:lower():find("cheat") or obj.Name:lower():find("kick") then
                    pcall(function()
                        obj.OnServerEvent:Destroy()
                        obj.OnClientEvent:Destroy()
                    end)
                end
            end
        end
        
        -- Disable teleport detection
        local teleportService = game:GetService("TeleportService")
        pcall(function()
            teleportService:SetTeleportGuiFadeInEnabled(false)
            teleportService:SetTeleportGuiFadeOutEnabled(false)
        end)
    end)
end
...
local function createStepSlider()
    local stepFrame = Instance.new("Frame")
    stepFrame.Size = UDim2.new(0, 190, 0, 34)
    stepFrame.Position = UDim2.new(0.5, -95, 0, 58)
    stepFrame.BackgroundColor3 = C.panelLight
    stepFrame.BackgroundTransparency = 0.1
    stepFrame.BorderSizePixel = 0
    stepFrame.Parent = buttonContainer
    
    local stepCorner = Instance.new("UICorner")
    stepCorner.CornerRadius = UDim.new(0, 8)
    stepCorner.Parent = stepFrame
    addRainbowStroke(stepFrame, 1.5, 0.6)
    
    local stepGlass = Instance.new("Frame")
    stepGlass.Size = UDim2.new(1, 0, 1, 0)
    stepGlass.BackgroundColor3 = C.glass
    stepGlass.BackgroundTransparency = 0.92
    stepGlass.BorderSizePixel = 0
    stepGlass.Parent = stepFrame
    local stepGlassCorner = Instance.new("UICorner")
    stepGlassCorner.CornerRadius = UDim.new(0, 8)
    stepGlassCorner.Parent = stepGlass
    
    local stepLabel = Instance.new("TextLabel")
    stepLabel.Size = UDim2.new(0, 70, 1, 0)
    stepLabel.Position = UDim2.new(0, 8, 0, 0)
    stepLabel.BackgroundTransparency = 1
    stepLabel.Text = "Steps:"
    stepLabel.TextColor3 = C.text
    ...
    stepInput.Size = UDim2.new(0, 60, 0, 24)
    stepInput.Position = UDim2.new(0, 80, 0.5, -12)
    stepInput.BackgroundColor3 = Color3.fromRGB(8, 8, 14)
    stepInput.Text = "50"
    stepInput.TextColor3 = C.primaryGlow
    stepInput.TextSize = 14
    stepInput.Font = Enum.Font.GothamBold
    stepInput.TextXAlignment = Enum.TextXAlignment.Center
    stepInput.BorderSizePixel = 0
    stepInput.Parent = stepFrame
    local stepInputCorner = Instance.new("UICorner")
    stepInputCorner.CornerRadius = UDim.new(0, 4)
    stepInputCorner.Parent = stepInput
    addRainbowStroke(stepInput, 1, 0.6)
    
    stepInput.FocusLost:Connect(function()
        local num = tonumber(stepInput.Text)
        if num and num > 0 then
            teleportSteps = math.floor(num)
            notify("📏 Steps set to: " .. teleportSteps, 2)
        else
            stepInput.Text = tostring(teleportSteps)
            notify("⚠️ Invalid input! Using: " .. teleportSteps, 2)
        end
    end)
...
buttonContainer.Position = UDim2.new(0.5, -110, 0.3, 0)
...
moveBtn.Size = UDim2.new(0, 190, 0, 40)
moveBtn.Position = UDim2.new(0.5, -95, 0, 8)
...
moveGlassCorner.CornerRadius = UDim.new(0, 10)
...
moveGlowCorner.CornerRadius = UDim.new(0, 12)
...
moveBtn.MouseButton1Click:Connect(function()
    pcall(function()
        ...
        local newPos = root.Position + lookVector * teleportSteps
        
        -- Anti-cheat bypass - smooth teleport
...
-- Create step slider
local stepSlider = createStepSlider()

-- BUTTON 2: TELEPORT TO CHOSEN
local teleportBtn = Instance.new("TextButton")
teleportBtn.Size = UDim2.new(0, 190, 0, 40)
teleportBtn.Position = UDim2.new(0.5, -95, 0, 130)
...
teleGlassCorner.CornerRadius = UDim.new(0, 10)
...
teleGlowCorner.CornerRadius = UDim.new(0, 12)
...
chooseContainer.Size = UDim2.new(0, 200, 0, 50)
chooseContainer.Position = UDim2.new(0.5, -100, 0.55, 0)
chooseContainer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
chooseContainer.BackgroundTransparency = 0.2
...
-- Draggable choose
local chooseDragging = false
local chooseDragOff = Vector2.new(0, 0)

pcall(function()
    chooseContainer.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then
            chooseDragging = true
            chooseDragOff = Vector2.new(mouse.X - chooseContainer.AbsolutePosition.X, mouse.Y - chooseContainer.AbsolutePosition.Y)
        end
    end)
end)
...
pcall(function()
    UserInputService.InputChanged:Connect(function(i)
        if chooseDragging and i.UserInputType == Enum.UserInputType.MouseMovement then
            pcall(function()
                chooseContainer.Position = UDim2.new(0, mouse.X - chooseDragOff.X, 0, mouse.Y - chooseDragOff.Y)
            end)
        end
    end)
end)
...
-- Update choose button color
local function updateChooseButton()
    if isChosen then
        chooseBtn.Text = "✅ POSITION CHOSEN!"
        chooseBtn.BackgroundColor3 = C.green
        chooseBtn.BackgroundTransparency = 0.1
    else
        chooseBtn.Text = "🔴 CHOOSE POSITION"
        chooseBtn.BackgroundColor3 = C.red
        chooseBtn.BackgroundTransparency = 0.1
    end
end

-- CHOOSE BUTTON CLICK - Toggles choice
...
chooseBtn.MouseButton2Click:Connect(function()
    chosenPosition = nil
    isChosen = false
    updateChooseButton()
    notify("🔄 Position reset!", 2)
end)

-- TELEPORT TO CHOSEN - Teleports to saved position
teleportBtn.MouseButton1Click:Connect(function()
    if not chosenPosition or not isChosen then
        notify("⚠️ No position chosen! Click the 'CHOOSE POSITION' button first.", 3)
        return
    end
    ...
    pcall(function()
        ...
        -- Smooth teleport with anti-cheat bypass
        local tweenInfo = TweenInfo.new(0.1, Enum.EasingStyle.Linear)
...
-- ==================================================
-- LOADING SCREEN PROGRESS - 7 SECONDS
-- ==================================================

task.spawn(function()
    local msgIndex = 1
    local totalSteps = 140 -- 7 seconds at 20 steps per second
    for i = 0, 100 do
        pcall(function()
        ...
        task.wait(0.05) -- 7 seconds total (100 * 0.05 = 5 seconds, plus extra)
    end
    
    -- Extra time to reach 7 seconds
    task.wait(2)
    ...
    buttonContainer.Visible = false
    notify("🔴 ReddieHub v23.1 Loaded! Click 🔴 to open", 4)
    print("✅ Loading screen destroyed!")
end)

print("==========================================")
print("✅ ReddieHub v23.1 - 2-BUTTON TELEPORT SYSTEM")
print("📋 Features:")
print("   🔴 7-Second Loading Screen")
print("   🔴 Anti-Cheat Bypass Active")
print("   🌈 Rainbow Borders on EVERYTHING")
print("   📦 Movable Button Container")
print("   🚀 Move Forward - Adjustable Steps")
print("   📍 Teleport to Chosen Position")
print("   ✅ Choose/Unchoose - Green/Red")
print("   💎 Best GUI Ever")
print("==========================================")
