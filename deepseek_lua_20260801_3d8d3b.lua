-- ==================================================
-- REDDIEHUB v25.0 - ULTIMATE TERMINAL EDITION
-- Floating Teleport • Skybox Changer • Font Changer
-- Key: Ali • Wavey Crystal/Snowflake Background
-- ==================================================
...
hub.Size = UDim2.new(0, 680, 0, 560)
hub.Position = UDim2.new(0.5, -340, 0.5, -280)
...
-- ==================================================
-- WAVEY CRYSTAL/SNOWFLAKE BACKGROUND
-- ==================================================
...
bgContainer.Parent = hub

-- Create animated crystal/snowflake particles
local particles = {}
for i = 1, 50 do
    ...
    particle.BackgroundTransparency = 0.3 + math.random() * 0.5
    particle.BorderSizePixel = 0
    particle.Parent = bgContainer
...
task.spawn(function()
    while bgContainer and bgContainer.Parent do
        for _, p in ipairs(particles) do
            p.wobble = p.wobble + 0.02
            local x = p.obj.Position.X.Scale + math.sin(p.wobble * p.wobbleSpeed) * 0.002
            local y = p.obj.Position.Y.Scale + p.speed * 0.002
            if y > 1 then y = 0 end
            if x > 1 then x = 0 end
            if x < 0 then x = 1 end
            pcall(function()
                p.obj.Position = UDim2.new(x, 0, y, 0)
                p.obj.BackgroundTransparency = 0.3 + math.sin(p.wobble * 0.5) * 0.2
            end)
        end
        task.wait(0.016)
    end
...
-- Glow overlay for text visibility
local glowOverlay = Instance.new("Frame")
glowOverlay.Size = UDim2.new(1, 0, 1, 0)
glowOverlay.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
glowOverlay.BackgroundTransparency = 0.5
glowOverlay.BorderSizePixel = 0
glowOverlay.Parent = bgContainer
...
-- ==================================================
-- TABS - Extended for new features
-- ==================================================
...
local tabNames = {"🏃 Move", "📍 Tele", "🎯 Lock", "👁️ ESP", "☁️ Skybox", "🔤 Fonts", "🎮 Tools", "📊 Stats"}
local tabBtns = {}
local tabPages = {}
...
-- ==================================================
-- FLOATING TELEPORT SYSTEM
-- ==================================================

local floatingEnabled = false
local floatingStep = 50
...
local function createFloatingButtons()
    ...
    floatingFrame.Size = UDim2.new(0, 180, 0, 120)
    floatingFrame.Position = UDim2.new(0.5, -90, 0.5, 60)
    floatingFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    floatingFrame.BackgroundTransparency = 0.2
    floatingFrame.BorderSizePixel = 2
    floatingFrame.BorderColor3 = C.primary
    ...
    addRetroStroke(floatingFrame, 2)
    
    -- Drag floating
    local fDragging = false
    local fDragOff = Vector2.new(0, 0)
    
    pcall(function()
        floatingFrame.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 then
                fDragging = true
                fDragOff = Vector2.new(mouse.X - floatingFrame.AbsolutePosition.X, mouse.Y - floatingFrame.AbsolutePosition.Y)
            end
        end)
    end)
    ...
    pcall(function()
        UserInputService.InputChanged:Connect(function(i)
            if fDragging and i.UserInputType == Enum.UserInputType.MouseMovement then
                pcall(function()
                    floatingFrame.Position = UDim2.new(0, mouse.X - fDragOff.X, 0, mouse.Y - fDragOff.Y)
                end)
            end
        end)
    end)
    ...
    fTitle.Position = UDim2.new(0, 0, 0, 2)
    fTitle.BackgroundTransparency = 1
    fTitle.Text = "> FLOATING TELEPORT"
    fTitle.TextColor3 = C.primary
    fTitle.TextSize = 10
    fTitle.Font = Enum.Font.Code
    fTitle.TextXAlignment = Enum.TextXAlignment.Center
    fTitle.Parent = floatingFrame
    
    -- Up button
    local upBtn = Instance.new("TextButton")
    upBtn.Size = UDim2.new(0, 50, 0, 30)
    upBtn.Position = UDim2.new(0.5, -55, 0, 24)
    upBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    upBtn.BackgroundTransparency = 0.3
    upBtn.BorderSizePixel = 1
    upBtn.BorderColor3 = C.primary
    upBtn.Text = "▲ UP"
    upBtn.TextColor3 = C.primary
    upBtn.TextSize = 12
    upBtn.Font = Enum.Font.Code
    upBtn.Parent = floatingFrame
    local upCorner = Instance.new("UICorner")
    upCorner.CornerRadius = UDim.new(0, 0)
    ...
    upBtn.MouseButton1Click:Connect(function()
        pcall(function()
            ...
            local newPos = root.Position + Vector3.new(0, floatingStep, 0)
            local tween = TweenService:Create(root, TweenInfo.new(0.1), {CFrame = CFrame.new(newPos)})
            tween:Play()
    ...
    downBtn.Position = UDim2.new(0.5, 5, 0, 24)
    downBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    downBtn.BackgroundTransparency = 0.3
    downBtn.BorderSizePixel = 1
    downBtn.BorderColor3 = C.primary
    downBtn.Text = "▼ DOWN"
    downBtn.TextColor3 = C.primary
    downBtn.TextSize = 12
    downBtn.Font = Enum.Font.Code
    downBtn.Parent = floatingFrame
    local downCorner = Instance.new("UICorner")
    downCorner.CornerRadius = UDim.new(0, 0)
    ...
    leftBtn.Position = UDim2.new(0.5, -55, 0, 58)
    leftBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    leftBtn.BackgroundTransparency = 0.3
    leftBtn.BorderSizePixel = 1
    leftBtn.BorderColor3 = C.primary
    leftBtn.Text = "◄ LEFT"
    leftBtn.TextColor3 = C.primary
    leftBtn.TextSize = 12
    leftBtn.Font = Enum.Font.Code
    leftBtn.Parent = floatingFrame
    ...
    leftBtn.MouseButton1Click:Connect(function()
        pcall(function()
            ...
            local newPos = root.Position + rightVector * -floatingStep
            local tween = TweenService:Create(root, TweenInfo.new(0.1), {CFrame = CFrame.new(newPos)})
            tween:Play()
            tween.Completed:Wait()
        end)
    ...
    rightBtn.Position = UDim2.new(0.5, 5, 0, 58)
    rightBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    rightBtn.BackgroundTransparency = 0.3
    rightBtn.BorderSizePixel = 1
    rightBtn.BorderColor3 = C.primary
    rightBtn.Text = "RIGHT ►"
    rightBtn.TextColor3 = C.primary
    rightBtn.TextSize = 12
    rightBtn.Font = Enum.Font.Code
    rightBtn.Parent = floatingFrame
    ...
    floatingButtons = {upBtn, downBtn, leftBtn, rightBtn}
...
local function changeSkybox(assetId)
    pcall(function()
        -- Try to change via Lighting
        if assetId == "reset" then
        ...
        -- Try to set skybox using Skybox object
        ...
        -- Get skybox asset
        local assetIdNum = tonumber(assetId)
        if assetIdNum then
            sky.SkyboxBk = "rbxassetid://" .. assetIdNum
            sky.SkyboxDn = "rbxassetid://" .. assetIdNum
            sky.SkyboxFt = "rbxassetid://" .. assetIdNum
            sky.SkyboxLf = "rbxassetid://" .. assetIdNum
            sky.SkyboxRt = "rbxassetid://" .. assetIdNum
            sky.SkyboxUp = "rbxassetid://" .. assetIdNum
            notify("☁️ Skybox changed successfully!", 2)
        end
    end)
end
...
local fontList = {
    {name = "Minecraft", font = Enum.Font.SourceSans},
    {name = "Gotham", font = Enum.Font.GothamBold},
    {name = "Code", font = Enum.Font.Code},
    {name = "Arial", font = Enum.Font.Arial},
    {name = "Roboto", font = Enum.Font.RobotoMono},
    {name = "Comic Sans", font = Enum.Font.ComicSansMs},
    {name = "Fantasy", font = Enum.Font.Fantasy},
    {name = "Garamond", font = Enum.Font.Garamond},
    {name = "Highway", font = Enum.Font.Highway},
    {name = "Bahnschrift", font = Enum.Font.Bahnschrift},
}
...
local function changeFont(font)
    pcall(function()
        ...
        -- Change for CoreGui (if possible)
        pcall(function()
        ...
        notify("🔤 Font changed to: " .. font.Name, 2)
    end)
end

-- ==================================================
-- TAB 1: MOVEMENT (Original + Floating)
-- ==================================================
...
-- Move Forward button
local moveBtn = button(p1, "[ MOVE FORWARD ] TAP=50 / HOLD=SLOW", function()
    pcall(function()
        local char = player.Character
        if not char then return end
        local root = char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        local lookVector = root.CFrame.LookVector
        local newPos = root.Position + lookVector * 50
...
local stepsSlider = slider(p1, "STEP COUNT", 1, 200, 50, function(v)
    floatingStep = math.floor(v)
end)

-- Floating toggle
local floatingToggle = toggle(p1, "FLOATING TELEPORT", false, function(s)
    floatingEnabled = s
    if s then
        if not floatingFrame then
            createFloatingButtons()
        end
        floatingFrame.Visible = true
        notify("🌀 Floating teleport enabled!", 2)
    else
        if floatingFrame then
            floatingFrame.Visible = false
        end
        notify("🌀 Floating teleport disabled!", 2)
    end
end)
...
local viewBtn = button(p2, "[ VIEW ]", function()
    ...
    viewCoroutine = coroutine.create(function()
        while viewTarget and viewTarget.Character do
            pcall(function()
                ...
                if camera and targetChar and targetChar:FindFirstChild("Head") then
                    ...
                    camera.CFrame = camera.CFrame:Lerp(CFrame.lookAt(currentPos, headPos), 0.12)
                end
            end)
            task.wait(0.016)
        end
...
RunService.RenderStepped:Connect(function()
    ...
    pcall(function()
        ...
        local smoothFactor = 1 / math.abs(camlockSmoothness)
        
        if math.abs(camlockSmoothness) <= 1 then
...
label(p5, "Click a button to change the skybox", C.gold)
label(p5, "Works in any game!", C.gold)

for _, asset in ipairs(skyboxAssets) do
    button(p5, "[ " .. asset.name .. " ]", function()
...
local p6 = tabPages[6]
section(p6, "UNIVERSAL FONT CHANGER")

label(p6, "Click a font to change the entire game's font", C.gold)
label(p6, "Works in any game - even on home page!", C.gold)
...
button(p6, "[ RESET FONT ]", function()
    changeFont(Enum.Font.SourceSans)
end, C.red)

-- ==================================================
-- TAB 7: TOOLS (Original)
-- ==================================================

local p7 = tabPages[7]

section(p7, "EMOTES HUB")
label(p7, "Click to open Emotes Hub", C.gold)
button(p7, "[ OPEN EMOTES HUB ]", function()
    pcall(function()
...
section(p7, "MACRO")
button(p7, "[ EXECUTE MACRO ]", function()
    pcall(function()
...
ftText.Text = "$ REDDIEHUB v25.0 // TERMINAL EDITION"
    ...
    pcall(function()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.KeyCode == Enum.KeyCode.RightShift then
                ...
                if hub.Visible then
                    pcall(function()
                        hub:TweenSize(UDim2.new(0, 680, 0, 560), "Out", "Back", 0.5, true)
                    end)
                    toggleBtn.Text = "🖥️"
...
print("==========================================")
print("✅ ReddieHub v25.0 - ULTIMATE TERMINAL EDITION")
print("📋 Features:")
print("   🖥️ Retro Terminal Style")
print("   🌀 Floating Teleport - 4 Directional")
print("   ☁️ Skybox Changer - 3 Presets + Reset")
print("   🔤 Universal Font Changer - 10 Fonts")
print("   🎯 Camlock - Smooth/Offset")
print("   👁️ ESP - Auto-updates")
print("   🎭 Emotes Hub + Macro")
print("   ⌨️ Keybinds: RightShift=Open, Q=Camlock")
print("   🌊 Wavey Crystal/Snowflake BG")
print("   💎 Best GUI Ever")
print("==========================================")
