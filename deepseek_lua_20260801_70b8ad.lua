...
-- Fixed Font Changer • Fixed Floating Buttons • Improved GUI
-- Crystal/Snowflake Theme • Key: Ali
-- ==================================================
...
hub.Size = UDim2.new(0, 640, 0, 600)
hub.Position = UDim2.new(0.5, -320, 0.5, -300)
...
toggleBtn.MouseButton1Click:Connect(function()
    ...
    if hub.Visible then
        pcall(function()
            hub:TweenSize(UDim2.new(0, 640, 0, 600), "Out", "Back", 0.5, true)
        end)
        toggleBtn.Text = "🟢"
...
local function updateTracer()
    ...
    pcall(function()
        ...
        if dist > 10 then
            ...
            tracerFrame.BackgroundColor3 = C.primary            tracerFrame.BackgroundTransparency = 0.3 + math.sin(tick() * 3) * 0.1
...
-- ==================================================
-- FLOATING BUTTONS SYSTEM - FIXED WITH STOP BUTTON
-- ==================================================
...
local function createFloatingButtons()
    ...
    fwdBtn.Size = UDim2.new(0, 60, 0, 60)
    fwdBtn.Position = UDim2.new(0.5, -30, 0.25, 0)
    ...
    bwdBtn.Size = UDim2.new(0, 60, 0, 60)
    bwdBtn.Position = UDim2.new(0.5, -30, 0.25 + spacing * 1, 0)
    ...
    lftBtn.Size = UDim2.new(0, 60, 0, 60)
    lftBtn.Position = UDim2.new(0.4, -30, 0.25 + spacing * 0.5, 0)
    ...
    rgtBtn.Size = UDim2.new(0, 60, 0, 60)
    rgtBtn.Position = UDim2.new(0.6, -30, 0.25 + spacing * 0.5, 0)
    ...
    upBtn.Position = UDim2.new(0.5, -30, 0.25 - spacing * 0.5, 0)
    ...
    downBtn.Position = UDim2.new(0.5, -30, 0.25 + spacing * 1.5, 0)
    ...
    stopBtn.Size = UDim2.new(0, 55, 0, 55)
    stopBtn.Position = UDim2.new(0.5, -27.5, 0.25 + spacing * 0.5, 0)
    ...
    stopCorner.CornerRadius = UDim.new(0, 16)
    stopCorner.Parent = stopBtn
    addCrystalStroke(stopBtn, 2)
    addGlowStroke(stopBtn, 2)
    
    -- STOP button click - disables floating and hides buttons
    stopBtn.MouseButton1Click:Connect(function()
        if floatingEnabled then
            toggleFloating(false)
            -- Also toggle the switch in the UI
            if floatToggleRef then
                floatToggleRef.set(false)
            end
            notify("✋ Floating buttons stopped!", 2)
        end
    end)
    
    buttons = {fwdBtn, bwdBtn, lftBtn, rgtBtn, upBtn, downBtn}
    ...
    -- Also make stop button draggable
    local stopDragging = false
    local stopDragOff = Vector2.new(0, 0)
    
    stopBtn.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then
            stopDragging = true
            stopDragOff = Vector2.new(mouse.X - stopBtn.AbsolutePosition.X, mouse.Y - stopBtn.AbsolutePosition.Y)
        end
    end)
    
    UserInputService.InputChanged:Connect(function(i)
        if stopDragging and i.UserInputType == Enum.UserInputType.MouseMovement then
            pcall(function()
                stopBtn.Position = UDim2.new(0, mouse.X - stopDragOff.X, 0, mouse.Y - stopDragOff.Y)
            end)
        end
    end)
...
local function toggleFloating(state)
    ...
    if stopBtn then
        stopBtn.Visible = state
    end
end

-- Store reference for toggle
local floatToggleRef = nil

-- ==================================================
-- FONT CHANGER - FIXED - INSTANT CHANGE
-- ==================================================

local selectedFont = "Gotham"
local fontUpdateConnections = {}

local function applyFontToObject(obj)
    if not obj then return end
    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
        pcall(function()
            local fontName = selectedFont
...
local function scanAndApplyFonts()
    -- CoreGui
    ...
    -- Workspace GUI elements
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BillboardGui") or obj:IsA("SurfaceGui") then
            for _, child in ipairs(obj:GetDescendants()) do
                applyFontToObject(child)
            end
        end
    end
end
...
-- Setup font watcher for new GUI elements
local function setupFontWatcher()
    -- Clean up old connections
    for _, conn in ipairs(fontUpdateConnections) do
        pcall(function() conn:Disconnect() end)
    end
    fontUpdateConnections = {}
    
    -- Watch for new GUI elements
    local function watchGui(gui)
        if not gui then return end
        local conn = gui.DescendantAdded:Connect(function(child)
            -- Apply font to new child and its descendants
            applyFontToObject(child)
            for _, desc in ipairs(child:GetDescendants()) do
                applyFontToObject(desc)
            end
        end)
        table.insert(fontUpdateConnections, conn)
    end
    ...
    -- Also watch CoreGui additions
    local coreConn = CoreGui.DescendantAdded:Connect(function(child)
        if child:IsA("ScreenGui") or child:IsA("BillboardGui") or child:IsA("SurfaceGui") then
            watchGui(child)
            -- Apply font to all existing children
            for _, desc in ipairs(child:GetDescendants()) do
                applyFontToObject(desc)
            end
        end
    ...
    table.insert(fontUpdateConnections, coreConn)
    
    -- Watch PlayerGui additions
    local playerConn = player.DescendantAdded:Connect(function(child)
        if child:IsA("ScreenGui") or child:IsA("BillboardGui") or child:IsA("SurfaceGui") then
            watchGui(child)
            for _, desc in ipairs(child:GetDescendants()) do
                applyFontToObject(desc)
    ...
    table.insert(fontUpdateConnections, playerConn)
end

-- Run initial font scan and setup watcher
scanAndApplyFonts()
setupFontWatcher()
...
for _, font in ipairs(fonts) do
    button(fontContainer, "🔤 " .. font, function()
        changeFont(font)
    end, C.secondary)
end
...
print("   🔤 Instant Universal Font Changer")
