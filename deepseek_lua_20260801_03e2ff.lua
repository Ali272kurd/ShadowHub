...
camlockBtn.MouseEnter:Connect(function()
    playClickSound()
end)
...
-- ==================================================
-- FLOATING BUTTONS SYSTEM - FIXED & BIGGER
-- ==================================================
...
local function createFloatingButtons()
    ...
    fwdBtn.MouseEnter:Connect(function()
        playClickSound()
    end)
    
    -- Backward button (Bigger)
    local bwdBtn = Instance.new("TextButton")
    bwdBtn.Size = UDim2.new(0, 65, 0, 65)
    bwdBtn.Position = UDim2.new(0.5, -32.5, 0.5, 0)
    ...
    bwdBtn.MouseEnter:Connect(function()
        playClickSound()
    end)
    ...
    lftBtn.Position = UDim2.new(0.4, -32.5, 0.4, 0)
    ...
    lftBtn.MouseEnter:Connect(function()
        playClickSound()
    end)
    ...
    rgtBtn.Position = UDim2.new(0.6, -32.5, 0.4, 0)
    ...
    rgtBtn.MouseEnter:Connect(function()
        playClickSound()
    end)
    ...
    upBtn.Position = UDim2.new(0.5, -32.5, 0.25, 0)
    ...
    downBtn.Position = UDim2.new(0.5, -32.5, 0.55, 0)
...
local skyboxes = {
    ...
    {"🌸 Cherry Blossom Skybox", 6549873210},
...
-- ==================================================
-- UNIVERSAL FONT CHANGER - FIXED (Instant)
-- ==================================================
...
local function applyFontToObject(obj)
    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
        pcall(function()
            ...
            elseif currentFont == "Times New Roman" then
            ...
            elseif currentFont == "Verdana" then
                obj.Font = Enum.Font.Arcade
            end
        end)
    end
...
-- Hook into new GUI elements being added
local function hookNewGuiElements()
    local function onDescendantAdded(obj)
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            applyFontToObject(obj)
        end
        -- Also hook children of this object
        obj.DescendantAdded:Connect(onDescendantAdded)
    end
    
    -- Hook CoreGui
    for _, gui in ipairs(CoreGui:GetChildren()) do
        gui.DescendantAdded:Connect(onDescendantAdded)
    end
...
-- Also apply every 0.5 seconds for any missed elements
coroutine.wrap(function()
    while true do
        task.wait(0.5)
        if currentFont then
...
local stopFloatBtn = button(p1, "⏹️ Stop Floating", function()
    toggleFloating(false)
    floatToggle.set(false)
    notify("⏹️ Floating buttons stopped!", 2)
end, C.red)
...
local p8 = tabPages[6] -- Stats is at index 6
