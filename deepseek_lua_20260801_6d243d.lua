-- ==================================================
-- REDDIEHUB v25.2 - ULTIMATE PREMIUM EDITION
-- Fixed Fonts • Skybox • Floating Buttons • Better GUI
-- Crystal/Snowflake Theme • Key: Ali • Button Sounds
-- ==================================================

-- Safety pcall for all operations
local success, err = pcall(function()

print("Loading ReddieHub v25.2...")
...
local function playButtonSound()
    pcall(function()
        ...
        sound.SoundId = "rbxassetid://9120281843"
        sound.Volume = 0.3
...
borderFrame.Size = UDim2.new(0, 420, 0, 480)
borderFrame.Position = UDim2.new(0.5, -210, 0.5, -240)
...
authTitle.Text = "REDDIEHUB v25.2"
authTitle.TextColor3 = C.primaryGlow
authTitle.TextSize = 32
...
avatar.Position = UDim2.new(0.5, -50, 0, 115)
...
usernameLabel.Position = UDim2.new(0, 20, 0, 225)
...
tiktokLabel.Position = UDim2.new(0, 20, 0, 255)
...
keyInput.Size = UDim2.new(0.8, 0, 0, 48)
keyInput.Position = UDim2.new(0.1, 0, 0, 320)
...
activateBtn.Size = UDim2.new(0.6, 0, 0, 48)
activateBtn.Position = UDim2.new(0.2, 0, 0, 385)
...
activateBtn.MouseButton1Click:Connect(function()
    ...
    if key == "Ali" then
        ...
        notify("🔷 ReddieHub v25.2 Loaded! Click 🔷 to open", 4)
...
-- ==================================================
-- MAIN HUB - Improved GUI
-- ==================================================

local hub = Instance.new("Frame")
hub.Size = UDim2.new(0, 650, 0, 600)
hub.Position = UDim2.new(0.5, -325, 0.5, -300)
...
titleGlassCorner.CornerRadius = UDim.new(0, 28)
...
closeB.Size = UDim2.new(0, 38, 0, 38)
closeB.Position = UDim2.new(1, -48, 0.5, -19)
...
toggleBtn.MouseButton1Click:Connect(function()
    ...
    if hub.Visible then
        pcall(function()
            hub:TweenSize(UDim2.new(0, 650, 0, 600), "Out", "Back", 0.5, true)
        end)
        toggleBtn.Text = "🟢"
...
tabC.Size = UDim2.new(1, -20, 0, 42)
tabC.Position = UDim2.new(0, 10, 0, 62)
...
contentGlassCorner.CornerRadius = UDim.new(0, 16)
...
local function colorPicker(parent, text, default, cb)
    ...
    colorBtn.MouseButton1Click:Connect(function()
        ...
        menu.Position = UDim2.new(0, 88, 0, 38)
...
local floatingStopRequested = false
...
camlockFrame.Size = UDim2.new(0, 90, 0, 90)
camlockFrame.Position = UDim2.new(0, 14, 0, 45)
...
camlockCorner.CornerRadius = UDim.new(0, 20)
...
camlockGlassCorner.CornerRadius = UDim.new(0, 20)
...
camlockGlowCorner.CornerRadius = UDim.new(0, 22)
...
-- ==================================================
-- FLOATING BUTTONS SYSTEM - FIXED & IMPROVED
-- ==================================================
...
local function createFloatingButtons()
    ...
    fwdBtn.Size = UDim2.new(0, 65, 0, 65)
    fwdBtn.Position = UDim2.new(0.5, -32.5, 0.25, 0)
    ...
    fwdCorner.CornerRadius = UDim.new(0, 16)
    fwdCorner.Parent = fwdBtn
    addCrystalStroke(fwdBtn, 2.5)
    addGlowStroke(fwdBtn, 2.5)
    
    -- Backward button (bottom)
    local bwdBtn = Instance.new("TextButton")
    bwdBtn.Size = UDim2.new(0, 65, 0, 65)
    bwdBtn.Position = UDim2.new(0.5, -32.5, 0.55, 0)
    ...
    bwdCorner.CornerRadius = UDim.new(0, 16)
    bwdCorner.Parent = bwdBtn
    addCrystalStroke(bwdBtn, 2.5)
    addGlowStroke(bwdBtn, 2.5)
    
    -- Left button
    local lftBtn = Instance.new("TextButton")
    lftBtn.Size = UDim2.new(0, 65, 0, 65)
    lftBtn.Position = UDim2.new(0.42, -32.5, 0.4, 0)
    ...
    lftCorner.CornerRadius = UDim.new(0, 16)
    lftCorner.Parent = lftBtn
    addCrystalStroke(lftBtn, 2.5)
    addGlowStroke(lftBtn, 2.5)
    
    -- Right button
    local rgtBtn = Instance.new("TextButton")
    rgtBtn.Size = UDim2.new(0, 65, 0, 65)
    rgtBtn.Position = UDim2.new(0.58, -32.5, 0.4, 0)
    ...
    rgtCorner.CornerRadius = UDim.new(0, 16)
    rgtCorner.Parent = rgtBtn
    addCrystalStroke(rgtBtn, 2.5)
    addGlowStroke(rgtBtn, 2.5)
    
    -- Up button
    local upBtn = Instance.new("TextButton")
    upBtn.Size = UDim2.new(0, 65, 0, 65)
    upBtn.Position = UDim2.new(0.5, -32.5, 0.18, 0)
    ...
    upCorner.CornerRadius = UDim.new(0, 16)
    upCorner.Parent = upBtn
    addCrystalStroke(upBtn, 2.5)
    addGlowStroke(upBtn, 2.5)
    
    -- Down button
    local downBtn = Instance.new("TextButton")
    downBtn.Size = UDim2.new(0, 65, 0, 65)
    downBtn.Position = UDim2.new(0.5, -32.5, 0.62, 0)
    ...
    downCorner.CornerRadius = UDim.new(0, 16)
    downCorner.Parent = downBtn
    addCrystalStroke(downBtn, 2.5)
    addGlowStroke(downBtn, 2.5)
        ...
        pcall(function()
            ...
            if char then
                ...
                if root then
                    ...
                    local tween = TweenService:Create(root, TweenInfo.new(0.05, Enum.EasingStyle.Linear), {CFrame = CFrame.new(newPos)})
                    tween:Play()
                    tween.Completed:Wait()
...
local stopFloatBtn = Instance.new("TextButton")
stopFloatBtn.Size = UDim2.new(0, 80, 0, 40)
stopFloatBtn.Position = UDim2.new(0.5, -40, 0.7, 0)
stopFloatBtn.BackgroundColor3 = C.red
stopFloatBtn.BackgroundTransparency = 0.1
stopFloatBtn.Text = "⏹ STOP"
stopFloatBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
stopFloatBtn.TextSize = 16
stopFloatBtn.Font = Enum.Font.GothamBold
stopFloatBtn.BorderSizePixel = 0
stopFloatBtn.Visible = false
stopFloatBtn.Parent = screenGui

local stopCorner = Instance.new("UICorner")
stopCorner.CornerRadius = UDim.new(0, 12)
stopCorner.Parent = stopFloatBtn
addCrystalStroke(stopFloatBtn, 2)
addGlowStroke(stopFloatBtn, 2)

stopFloatBtn.MouseButton1Click:Connect(function()
    playButtonSound()
    floatingEnabled = false
    floatingStopRequested = true
    for _, btn in ipairs(floatingBtns) do
        btn.Visible = false
    end
    stopFloatBtn.Visible = false
    notify("⏹ Floating buttons stopped!", 2)
end)

-- Show stop button when floating is enabled
local function updateStopButton()
    stopFloatBtn.Visible = floatingEnabled and not floatingStopRequested
end

-- Hook into toggle
local originalToggleFloating = toggleFloating
toggleFloating = function(state)
    originalToggleFloating(state)
    if state then
        floatingStopRequested = false
        stopFloatBtn.Visible = true
    else
        stopFloatBtn.Visible = false
    end
end
...
local skyboxes = {
    ...
    {"🌠 Shooting Star Skybox", 7531594860},
}

-- ==================================================
-- FONT CHANGER - FIXED - Instant apply, no delay
-- ==================================================

local selectedFont = nil
local fontApplyCoroutine = nil

local function applyFontToObject(obj, fontName)
...
local function scanAndApplyFont(fontName)
    pcall(function()
        -- Apply to all existing GUI elements
...
local function changeFont(fontName)
    ...
    -- Apply instantly
    scanAndApplyFont(fontName)
    
    -- Stop old coroutine if exists
    if fontApplyCoroutine then
        coroutine.close(fontApplyCoroutine)
        fontApplyCoroutine = nil
    end
    
    -- Start new continuous coroutine for new GUI elements
    fontApplyCoroutine = coroutine.create(function()
        while true do
            task.wait(0.5)
            if selectedFont then
                scanAndApplyFont(selectedFont)
            end
        end
    end)
    coroutine.resume(fontApplyCoroutine)
...
local floatToggle = toggle(p1, "Enable Floating Buttons", false, function(s)
    ...
    if s then
        stopFloatBtn.Visible = true
        notify("🌀 Floating buttons enabled", 2)
    else
        stopFloatBtn.Visible = false
        notify("🌀 Floating buttons disabled", 2)
    end
end)
...
label(p3, "Positive = Smoothness | Negative = Offset (Goes Before Target)", C.gold)
label(p3, "PC Keybind: Q", C.gold)

local comboFrame = Instance.new("Frame")
comboFrame.Size = UDim2.new(1, 0, 0, 36)
...
comboInput.FocusLost:Connect(function()
    ...
    if num then
        ...
        if num < 0 then
            offsetLabel.Text = "O:" .. string.format("%+.1f", num) .. " (Before Target)"
            offsetLabel.TextColor3 = C.red
            notify("📏 Offset set to: " .. string.format("%+.1f", num) .. " (Aim before target)", 2)
        else
...
bpFrame.Size = UDim2.new(1, 0, 0, 30)
...
button(p3, "🎯 Force Lock", function()
    ...
    if target then
        ...
        offsetLabel.Visible = true
        if camlockSmoothness < 0 then
            offsetLabel.Text = "O:" .. string.format("%+.1f", camlockSmoothness) .. " (Before Target)"
            offsetLabel.TextColor3 = C.red
        else
            offsetLabel.Text = "S:" .. tostring(math.floor(camlockSmoothness))
            offsetLabel.TextColor3 = C.green
...
local function createPremiumESP(char, plr)
    ...
    healthBar.Size = UDim2.new(0, 65, 0, 5)
    ...
    healthBg.BorderColor3 = Color3.fromRGB(50, 50, 50)
    ...
    nameLabel.StudsOffset = Vector3.new(0, 3.6, 0)
    ...
    box.Size = Vector3.new(2.6, 3.8, 1.2)
    ...
    boxGlow.Size = Vector3.new(2.8, 4.0, 1.4)
    boxGlow.Adornee = box.Adornee
    boxGlow.ZIndex = 0
    boxGlow.AlwaysOnTop = true
    boxGlow.Color3 = espColor
    boxGlow.Transparency = 0.7
    boxGlow.Parent = espGroup
    ...
    distText.Text = "0m"
    distText.TextColor3 = Color3.fromRGB(100, 100, 120)
    distText.TextSize = 8
    distText.Font = Enum.Font.GothamSemibold
    distText.Parent = distLabel
    
    -- Skeleton ESP
    local function createSkeleton(part1, part2, color)
        local connection = RunService.RenderStepped:Connect(function()
            ...
            if p1 and p2 then
                ...
                local line = espGroup:FindFirstChild("Skel_" .. part1 .. "_" .. part2)
                if not line then
                    line = Instance.new("BillboardGui")
                    line.Name = "Skel_" .. part1 .. "_" .. part2
                    line.Size = UDim2.new(0, dist, 0, 2)
                    line.StudsOffset = (p1.Position - midPoint)
                    line.Adornee = char:FindFirstChild("HumanoidRootPart")
                    line.MaxDistance = espDistance
                    line.Parent = espGroup
                    
                    local lineFrame = Instance.new("Frame")
                    lineFrame.Size = UDim2.new(1, 0, 1, 0)
                    lineFrame.BackgroundColor3 = color or Color3.fromRGB(0, 200, 255)
    ...
    createSkeleton("Head", "UpperTorso", Color3.fromRGB(0, 200, 255))
    createSkeleton("UpperTorso", "LowerTorso", Color3.fromRGB(0, 200, 255))
    createSkeleton("UpperTorso", "LeftUpperArm", Color3.fromRGB(0, 200, 255))
    createSkeleton("UpperTorso", "RightUpperArm", Color3.fromRGB(0, 200, 255))
    createSkeleton("LeftUpperArm", "LeftLowerArm", Color3.fromRGB(0, 200, 255))
    createSkeleton("RightUpperArm", "RightLowerArm", Color3.fromRGB(0, 200, 255))
    createSkeleton("UpperTorso", "LeftUpperLeg", Color3.fromRGB(0, 200, 255))
    createSkeleton("UpperTorso", "RightUpperLeg", Color3.fromRGB(0, 200, 255))
    createSkeleton("LeftUpperLeg", "LeftLowerLeg", Color3.fromRGB(0, 200, 255))
    createSkeleton("RightUpperLeg", "RightLowerLeg", Color3.fromRGB(0, 200, 255))
    ...
    local connection = RunService.RenderStepped:Connect(function()
        ...
        box.Size = Vector3.new(2.6 * scale, 3.8 * scale, 1.2 * scale)
        boxGlow.Size = Vector3.new(2.8 * scale, 4.0 * scale, 1.4 * scale)
...
label(p4, "• Clean Box ESP with Glow", C.text)
label(p4, "• Health bars", C.text)
label(p4, "• Dynamic scaling", C.text)
label(p4, "• Distance tracker", C.text)
label(p4, "• Skeleton ESP", C.text)
...
skyboxContainer.Size = UDim2.new(1, 0, 0, #skyboxes * 42 + 10)
skyboxContainer.BackgroundTransparency = 1
skyboxContainer.BorderSizePixel = 0
skyboxContainer.Parent = p6

local skyboxLayout = Instance.new("UIListLayout")
skyboxLayout.Padding = UDim.new(0, 4)
skyboxLayout.SortOrder = Enum.SortOrder.LayoutOrder
skyboxLayout.Parent = skyboxContainer

for _, skybox in ipairs(skyboxes) do
    button(skyboxContainer, skybox[1], function()
        playButtonSound()
        changeSkybox(skybox[2])
    end, C.primary)
end

button(skyboxContainer, "🔄 Reset Skybox", function()
    playButtonSound()
    resetSkybox()
end, C.red)

-- ==================================================
-- TAB 7: FONT CHANGER - FIXED
-- ==================================================
...
ftText.Text = "🔷 ReddieHub v25.2 • Made by aliiiwyddd"
...
print("==========================================")
print("✅ ReddieHub v25.2 - ULTIMATE PREMIUM EDITION")
print("📋 Features:")
print("   🔷 Crystal/Snowflake Theme")
print("   🎯 Camlock - Negative Offset = Before Target")
print("   👁️ Perfect Viewer")
print("   📊 Camlock Popup with Stats")
print("   📍 Perfect Tracer Line")
print("   👁️ ESP with Skeleton & Glow")
print("   🎨 Premium Visuals")
print("   🎭 Emotes Hub")
print("   🌤️ 25+ Skybox Changer")
print("   🔤 Universal Font Changer (Instant)")
print("   🌀 Floating Step Buttons with Stop")
print("   🔘 Button Sounds")
