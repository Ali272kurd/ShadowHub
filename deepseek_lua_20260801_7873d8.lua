...
-- Maclib Library • Crystal/Snowflake Theme
-- Floating Buttons • Skybox Changer • Font Changer
-- ==================================================

-- Load Maclib Library
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/x2Swiftz/UI-Library/main/Libraries/Maclib.lua"))()

-- Safety pcall for all operations
local success, err = pcall(function()

print("Loading ReddieHub v25.2 with Maclib...")
...
-- ==================================================
-- MACLIB UI - Main Window
-- ==================================================

local win = library:CreateWindow("ReddieHub v25.2", "Crystal Edition")

-- ==================================================
-- CREATE CRYSTAL BACKGROUND FOR MACLIB
-- ==================================================

local function createCrystalBackground()
    local screenGui = player:WaitForChild("PlayerGui"):FindFirstChild("ReddieHub")
...
-- Add crystal background after UI loads
task.spawn(function()
    task.wait(0.5)
...
camlockFrame.Parent = player:WaitForChild("PlayerGui")
...
camlockPopup.Parent = player:WaitForChild("PlayerGui")
...
tracerFrame.Parent = player:WaitForChild("PlayerGui")
...
local function createFloatingButtons()
    ...
    local directions = {
        {text = "↑", color = C.primary, x = 0.5, y = 0.35, label = "Forward"},
        {text = "↓", color = C.secondary, x = 0.5, y = 0.45, label = "Backward"},
        {text = "←", color = C.accent, x = 0.45, y = 0.4, label = "Left"},
        {text = "→", color = C.green, x = 0.55, y = 0.4, label = "Right"},
        {text = "⤊", color = C.blue, x = 0.5, y = 0.3, label = "Up"},
        {text = "⤋", color = C.purple, x = 0.5, y = 0.5, label = "Down"},
    }
    
    for _, dir in ipairs(directions) do
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, 55, 0, 55)
        btn.Position = UDim2.new(dir.x, -27.5, dir.y, 0)
        btn.BackgroundColor3 = dir.color
...
floatingBtns = createFloatingButtons()
...
moveTab:CreateButton("🚀 Inject Mooze Mob", function()
    pcall(function()
...
moveTab:CreateSection("Floating Controls")
moveTab:CreateToggle("Enable Floating Buttons", false, function(state)
    toggleFloating(state)
    notify(state and "🌀 Floating buttons enabled" or "🌀 Floating buttons disabled", 2)
end)

moveTab:CreateSection("Step Control")
moveTab:CreateSlider("Step Count", 1, 200, 50, function(value)
    teleportSteps = math.floor(value)
end)

-- Tab 2: Teleport
local teleTab = win:CreateTab("Teleport")

teleTab:CreateSection("Smart Teleport")
teleTab:CreateParagraph("Partial names work!")

local tpInput = teleTab:CreateInput("Username", "Enter username...", function(value) end)

teleTab:CreateButton("👁️ View", function()
    ...
    local target = findPlayer(tpInput:GetValue())
    if not target then 
        notify("❌ Not found!", 3) 
        return 
    end
...
teleTab:CreateButton("📍 Teleport", function()
    local target = findPlayer(tpInput:GetValue())
    if not target then 
        notify("❌ Not found!", 3) 
        return 
...
teleTab:CreateButton("🪑 Headsit", function()
    local target = findPlayer(tpInput:GetValue())
...
teleTab:CreateButton("ℹ️ Info", function()
    local target = findPlayer(tpInput:GetValue())
    if not target then notify("❌ Not found!", 3) return end
...
teleTab:CreateSection("Chosen Position")
local chooseBtn = teleTab:CreateButton("[ CHOOSE POSITION ]", function()
    pcall(function()
        local char = player.Character
        if char then
            local root = char:FindFirstChild("HumanoidRootPart")
            if root then
                ...
                isChosen = true
                chooseBtn:SetText("[ ✓ POSITION CHOSEN ]")
...
teleTab:CreateButton("[ UNCHOOSE ]", function()
    chosenPosition = nil
    isChosen = false
    chooseBtn:SetText("[ CHOOSE POSITION ]")
    notify("🔄 Position unchosen!", 2)
end)

teleTab:CreateButton("[ TELEPORT TO CHOSEN ]", function()
    teleportToChosen()
end)

-- Tab 3: Camlock
local camTab = win:CreateTab("Camlock")

camTab:CreateSection("Camlock Settings")
camTab:CreateParagraph("Positive = Smoothness | Negative = Offset (Before Target)")
camTab:CreateParagraph("PC Keybind: Q")

camTab:CreateSlider("Smoothness/Offset", -30, 30, 8, function(value)
    camlockSmoothness = value
    if value < 0 then
        offsetLabel.Text = "O:" .. string.format("%+.1f", value)
        offsetLabel.TextColor3 = C.red
    else
        offsetLabel.Text = "S:" .. tostring(math.floor(value))
        offsetLabel.TextColor3 = C.green
    end
end)

camTab:CreateSection("Body Part")
local bodyParts = {"Head", "Torso", "UpperTorso", "LowerTorso", "HumanoidRootPart"}
local bpDropdown = camTab:CreateDropdown("Select Body Part", bodyParts, function(value)
    camlockBodyPart = value
    notify("🎯 Body part: " .. value, 2)
end)

camTab:CreateSection("Controls")
camTab:CreateButton("🔄 Reset", function()
    camlockEnabled = false
    camlockTarget = nil
    camlockBtn.Text = "🔒"
    camlockBtn.TextColor3 = C.dim
...
camTab:CreateButton("🎯 Force Lock", function()
...
camTab:CreateButton("👁️ Show/Hide Camlock", function()
...
-- Tab 4: ESP
local espTab = win:CreateTab("ESP")
...
espTab:CreateSlider("Box Transparency", 0.2, 0.9, 0.5, function(value)
...
-- Tab 5: Tools
local toolsTab = win:CreateTab("Tools")

toolsTab:CreateSection("Emotes Hub")
toolsTab:CreateButton("🎭 Open Emotes Hub", function()
    pcall(function()
...
toolsTab:CreateSection("Visuals (Premium)")
toolsTab:CreateSlider("Brightness", 0.1, 2, 1, function(v)
    visualsBrightness = v
    applyVisuals()
end)

toolsTab:CreateSlider("Contrast", 0.1, 2, 1, function(v)
    visualsContrast = v
    applyVisuals()
end)

toolsTab:CreateSlider("Saturation", 0.1, 2, 1.2, function(v)
    visualsSaturation = v
    applyVisuals()
end)

toolsTab:CreateSlider("Bloom", 0, 1, 0.6, function(v)
    visualsBloom = v
    applyVisuals()
end)

toolsTab:CreateSlider("Color Temp (Warm/Cool)", -1, 1, 0, function(v)
    visualsColorTemp = v
    applyVisuals()
end)

toolsTab:CreateSlider("Exposure", 0, 1, 0.6, function(v)
    visualsExposure = v
    applyVisuals()
end)

toolsTab:CreateButton("🔄 Reset Visuals", function()
    visualsBrightness = 1
    visualsContrast = 1
...
toolsTab:CreateSection("Macro")
toolsTab:CreateButton("▶️ Execute Macro", function()
    pcall(function()
...
-- Tab 6: Skybox
local skyTab = win:CreateTab("Skybox")

skyTab:CreateSection("Skybox Changer")

for _, skybox in ipairs(skyboxes) do
    skyTab:CreateButton(skybox[1], function()
        changeSkybox(skybox[2])
    end)
end

skyTab:CreateButton("🔄 Reset Skybox", function()
    resetSkybox()
end)

-- Tab 7: Fonts
local fontTab = win:CreateTab("Fonts")

fontTab:CreateSection("Font Changer")
fontTab:CreateParagraph("Change font for the entire game")

for _, font in ipairs(fonts) do
    fontTab:CreateButton("🔤 " .. font, function()
        changeFont(font)
    end)
end
...
local pingLabel = statsTab:CreateLabel("📡 Ping: 0 ms")
local memLabel = statsTab:CreateLabel("💾 Memory: 0 MB")
...
print("==========================================")
print("✅ ReddieHub v25.2 - MACLIB EDITION")
print("📋 Features:")
