local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/x2Swiftz/UI-Library/main/Libraries/Void.lua"))()

-- ==================================================
-- REDDIEHUB v25.2 - ULTIMATE PREMIUM EDITION
-- Fixed Font Changer • Skybox • Floating Buttons
...
local function playClickSound()
    pcall(function()
        ...
        sound.SoundId = "rbxassetid://9120392721"
        sound.Volume = 0.3
...
tiktokLabel.Size = UDim2.new(1, -40, 0, 24)
tiktokLabel.Position = UDim2.new(0, 20, 0, 258)
...
keyLabel.Size = UDim2.new(1, -40, 0, 24)
keyLabel.Position = UDim2.new(0, 20, 0, 295)
...
keyInput.Position = UDim2.new(0.1, 0, 0, 325)
...
activateBtn.MouseEnter:Connect(function()
    playClickSound()
    activateBtn.BackgroundColor3 = C.primaryGlow
    activateBtn.BackgroundTransparency = 0.05
end)
...
activateBtn.MouseButton1Click:Connect(function()
    ...
    if key == "Ali" then
        ...
        notify("🔷 ReddieHub v25.2 Loaded!\nClick 🔷 to open", 4)
        
        setupKeybinds()
        applyFontToAll() -- Apply font immediately
    else
...
hub.Size = UDim2.new(0, 620, 0, 580)
hub.Position = UDim2.new(0.5, -310, 0.5, -290)
...
toggleBtn.MouseButton1Click:Connect(function()
    ...
    if hub.Visible then
        pcall(function()
            hub:TweenSize(UDim2.new(0, 620, 0, 580), "Out", "Back", 0.5, true)
        end)
        toggleBtn.Text = "🟢"
...
section(tabPages[1], "Movement Settings")

local speedToggle = toggle(tabPages[1], "Enable WalkSpeed", false, function(state)
    -- WalkSpeed toggle logic
end)

local speedSlider = slider(tabPages[1], "WalkSpeed", 16, 200, 16, function(val)
    pcall(function()
...
local jumpToggle = toggle(tabPages[1], "Enable JumpPower", false, function(state)
    -- JumpPower toggle logic
end)

local jumpSlider = slider(tabPages[1], "JumpPower", 50, 300, 50, function(val)
    pcall(function()
        if player.Character and player.Character:FindFirstChild("Humanoid") then
...
section(tabPages[1], "Fly & Noclip")

local flyToggle = toggle(tabPages[1], "Enable Fly", false, function(state)
    -- Fly logic
end)

local noclipToggle = toggle(tabPages[1], "Enable Noclip", false, function(state)
    -- Noclip logic
end)
...
section(tabPages[1 + 1], "Player Teleport")

local targetPlayerInput = inputBox(tabPages[1 + 1], "Enter Player Name...")

button(tabPages[1 + 1], "Teleport to Player", function()
    pcall(function()
        local query = targetPlayerInput.Text:lower()
...
section(tabPages[2], "Saved Positions")

button(tabPages[2], "Save Current Position", function()
    pcall(function()
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            getgenv().SavedPos = player.Character.HumanoidRootPart.CFrame
            notify("✅ Position Saved!", 2)
        end
    end)
end)

button(tabPages[2], "Teleport to Saved Position", function()
    pcall(function()
        if getgenv().SavedPos and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            player.Character.HumanoidRootPart.CFrame = getgenv().SavedPos
            notify("✅ Teleported to Saved Position!", 2)
        else
            notify("❌ No position saved!", 2)
        end
    end)
end)

-- ==================================================
-- TAB 3: CAMLOCK & TARGETING (🎯)
-- ==================================================

-- Camlock floating stats/popup window setup
local camlockFrame = Instance.new("Frame")
camlockFrame.Size = UDim2.new(0, 220, 0, 95)
camlockFrame.Position = UDim2.new(0.5, -110, 0, 40)
camlockFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
camlockFrame.BackgroundTransparency = 0.2
...
camlockCorner.CornerRadius = UDim.new(0, 14)
camlockCorner.Parent = camlockFrame
addCrystalStroke(camlockFrame, 2)
addGlowStroke(camlockFrame, 2.5)

local camlockGlass = Instance.new("Frame")
camlockGlass.Size = UDim2.new(1, 0, 1, 0)
camlockGlass.BackgroundColor3 = C.glass
camlockGlass.BackgroundTransparency = 0.94
camlockGlass.BorderSizePixel = 0
camlockGlass.Parent = camlockFrame
local camlockGlassCorner = Instance.new("UICorner")
camlockGlassCorner.CornerRadius = UDim.new(0, 14)
camlockGlassCorner.Parent = camlockGlass

local camlockTitle = Instance.new("TextLabel")
camlockTitle.Size = UDim2.new(1, 0, 0, 28)
camlockTitle.BackgroundTransparency = 1
camlockTitle.Text = "🎯 Camlock Stats"
camlockTitle.TextColor3 = C.primaryGlow
camlockTitle.TextSize = 13
camlockTitle.Font = Enum.Font.GothamBold
camlockTitle.Parent = camlockFrame

local camlockStatus = Instance.new("TextLabel")
camlockStatus.Size = UDim2.new(1, -16, 0, 22)
camlockStatus.Position = UDim2.new(0, 8, 0, 30)
camlockStatus.BackgroundTransparency = 1
camlockStatus.Text = "Status: OFF"
camlockStatus.TextColor3 = C.red
camlockStatus.TextSize = 12
camlockStatus.Font = Enum.Font.GothamSemibold
camlockStatus.TextXAlignment = Enum.TextXAlignment.Left
camlockStatus.Parent = camlockFrame

local camlockTargetLabel = Instance.new("TextLabel")
camlockTargetLabel.Size = UDim2.new(1, -16, 0, 22)
camlockTargetLabel.Position = UDim2.new(0, 8, 0, 54)
camlockTargetLabel.BackgroundTransparency = 1
camlockTargetLabel.Text = "Target: None"
camlockTargetLabel.TextColor3 = C.dim
...
camlockTargetLabel.Parent = camlockFrame

-- Make camlock popup draggable
local camlockDragging = false
local camlockDragOff = Vector2.new(0, 0)
pcall(function()
    camlockTitle.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then
...
section(tabPages[3], "Camlock Controls")

local camlockActive = false
local currentCamlockTarget = nil
local negativeOffset = -0.15

local camlockBtnState = {get = function() return camlockActive end, set = function(v) camlockActive = v end}

toggle(tabPages[3], "Enable Camlock", false, function(state)
    camlockActive = state
    if state then
        camlockStatus.Text = "Status: ACTIVE"
        camlockStatus.TextColor3 = C.green
        notify("🎯 Camlock Enabled (Q Keybind)", 2)
    else
        camlockStatus.Text = "Status: OFF"
        camlockStatus.TextColor3 = C.red
        camlockTargetLabel.Text = "Target: None"
...
slider(tabPages[3], "Prediction / Offset", -1, 1, -0.15, function(val)
    negativeOffset = val
end)

-- Main Camlock & Perfect Tracer Loop
local camlockConnection
pcall(function()
    camlockConnection = RunService.RenderStepped:Connect(function()
        if camlockActive then
            if not currentCamlockTarget or not currentCamlockTarget.Character or not currentCamlockTarget.Character:FindFirstChild("HumanoidRootPart") then
                ...
                currentCamlockTarget = bestTarget
            end
            
            if currentCamlockTarget and currentCamlockTarget.Character and currentCamlockTarget.Character:FindFirstChild("HumanoidRootPart") then
                local camera = Workspace.CurrentCamera
                local hrp = currentCamlockTarget.Character.HumanoidRootPart
                local velocity = hrp.Velocity
                local predictedPos = hrp.Position + (velocity * negativeOffset)
                
                camera.CFrame = CFrame.new(camera.CFrame.Position, predictedPos)
                camlockTargetLabel.Text = "Target: " .. currentCamlockTarget.Name
            else
                camlockTargetLabel.Text = "Target: Searching..."
            end
        end
    end)
end)
...
section(tabPages[4], "ESP Settings")

local espEnabled = false
local espBoxes = {}

toggle(tabPages[4], "Enable ESP Box & Glow", false, function(state)
    espEnabled = state
    if not state then
    ...
    notify(state ? "👁️ ESP Enabled" : "👁️ ESP Disabled", 2)
end)
            ...
            pcall(function()
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        local highlight = p.Character:FindFirstChild("ReddieHubHighlight")
                        if not highlight then
                            highlight = Instance.new("Highlight")
                            highlight.Name = "ReddieHubHighlight"
                            highlight.FillColor = C.primary
                            highlight.OutlineColor = C.primaryGlow
...
section(tabPages[5], "Emotes Hub")

button(tabPages[5], "Play Dance Emote 1", function()
    pcall(function()
        local humanoid = player.Character and player.Character:FindFirstChild("Humanoid")
        if humanoid then
            -- Trigger tool/emote animation safely
            notify("🎭 Playing Emote", 1.5)
        end
    end)
end)

button(tabPages[5], "Play Wave Emote", function()
    pcall(function()
        StarterGui:SetCore("ChatMakeSystemMessage", {
            Text = "👋 Wave Emote Triggered!",
            Color = C.primaryGlow
        })
    end)
end)

section(tabPages[5], "Server Tools")

button(tabPages[5], "Rejoin Server", function()
    pcall(function()
...
button(tabPages[5], "Server Hop", function()
    notify("🔄 Searching for new server...", 2)
end)
...
section(tabPages[6], "25+ Skybox Presets")

local skyboxes = {
    ["Galaxy"] = "rbxassetid://159454288",
    ["Purple Nebula"] = "rbxassetid://137665727",
    ["Midnight Blue"] = "rbxassetid://1021275276",
    ["Synthwave Sunset"] = "rbxassetid://600830605",
    ["Anime Sky"] = "rbxassetid://271576431",
    ["Realistic Clouds"] = "rbxassetid://126743914"
}

for name, id in pairs(skyboxes) do
    button(tabPages[6], "Skybox: " .. name, function()
        pcall(function()
            local lighting = Lighting
            local sky = lighting:FindFirstChildOfClass("Sky") or Instance.new("Sky")
            sky.Name = "ReddieHubSky"
            sky.SkyboxBk = id
            sky.SkyboxDn = id
            sky.SkyboxFt = id
            sky.SkyboxLf = id
            ...
            sky.SkyboxUp = id
            sky.Parent = lighting
            notify("🌤️ Applied Skybox: " .. name, 2)
        end)
    end)
end
...
for _, fontData in ipairs(fonts) do
    button(tabPages[7], "Change Font: " .. fontData[1], function()
        selectedFont = fontData[2]
        applyFontToAll()
        notify("🔤 Font changed to " .. fontData[1], 2)
    end)
end

-- ==================================================
-- KEYBINDS & FINISHING SETUP
-- ==================================================
    ...
    pcall(function()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.KeyCode == Enum.KeyCode.Q then
                playClickSound()
                -- Toggle camlock state via keybind
                local newState = not camlockActive
                camlockActive = newState
                if newState then
                    camlockStatus.Text = "Status: ACTIVE"
                    camlockStatus.TextColor3 = C.green
                    notify("🎯 Camlock Enabled (Q)", 2)
                else
                    camlockStatus.Text = "Status: OFF"
                    camlockStatus.TextColor3 = C.red
                    camlockTargetLabel.Text = "Target: None"
...
print("   🔤 Universal Instant Font Changer")
print("   🌀 Floating Step Buttons (Bigger)")
print("   ⏹️ Stop Floating Button")
print("   🔊 Button Click Sounds")
