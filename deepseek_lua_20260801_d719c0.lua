...
-- Key: Ali • Black & White Terminal Style
-- ==================================================
...
hub.Size = UDim2.new(0, 650, 0, 550)
hub.Position = UDim2.new(0.5, -325, 0.5, -275)
...
tabBg.Image = "rbxassetid://74795700105848"
tabBg.ScaleType = Enum.ScaleType.Fit
tabBg.Visible = false
tabBg.Parent = content

-- Tab system
local tabNames = {"🏃 Move", "📍 Tele", "🎯 Lock", "👁️ ESP", "🎮 Tools", "📊 Stats", "🌌 Skybox", "🔤 Fonts"}
local tabBtns = {}
local tabPages = {}
...
for i, name in ipairs(tabNames) do
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Show background for tabs 7 and 8
        if i == 7 or i == 8 then
            tabBg.Visible = true
        else
            tabBg.Visible = false
...
local floatStep = 10
local floatDirection = 1 -- 1 = forward, -1 = backward
...
local function createFloatButton()
    ...
    pcall(function()
        floatButton.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 then
                floatDragging = true
                floatDragOff = Vector2.new(mouse.X - floatButton.AbsolutePosition.X, mouse.Y - floatButton.AbsolutePosition.Y)
            end
        end)
    end)
    ...
    pcall(function()
        UserInputService.InputChanged:Connect(function(i)
            if floatDragging and i.UserInputType == Enum.UserInputType.MouseMovement then
                pcall(function()
                    floatButton.Position = UDim2.new(0, mouse.X - floatDragOff.X, 0, mouse.Y - floatDragOff.Y)
                end)
            end
        end)
    end)
    ...
    floatButton.MouseButton1Click:Connect(function()
        if floatEnabled then
            pcall(function()
                ...
                -- Combine directions based on button text
                local dir = Vector3.new(0, 0, 0)
                local text = floatButton.Text
                if text == "⬆" then
                    dir = lookVector * floatStep * floatDirection
                elseif text == "⬇" then
                    dir = -lookVector * floatStep * floatDirection
                elseif text == "⬅" then
                    dir = -rightVector * floatStep
                elseif text == "➡" then
                    dir = rightVector * floatStep
                elseif text == "⬆⬆" then
                    dir = upVector * floatStep
                elseif text == "⬇⬇" then
                    dir = -upVector * floatStep
                end
                
                local newPos = root.Position + dir
...
-- Camlock button with custom image
local camlockBtn = Instance.new("ImageButton")
camlockBtn.Size = UDim2.new(0.8, 0, 0.8, 0)
camlockBtn.Position = UDim2.new(0.1, 0, 0.1, 0)
camlockBtn.BackgroundTransparency = 1
camlockBtn.Image = "rbxassetid://126049593669150"
camlockBtn.ScaleType = Enum.ScaleType.Fit
...
camlockBtn.MouseButton1Click:Connect(function()
    ...
    if camlockEnabled then
        ...
        if camlockTarget then
            camlockBtn.ImageColor3 = C.green
            camlockFrame.BorderColor3 = C.green
...
local function changeSkybox(assetId)
    pcall(function()
        ...
        if assetId == "reset" then
            ...
            sky.SkyboxRt = ""
            sky.SkyboxUp = ""
            notify("🌤️ Skybox reset to default!", 2)
        else
...
local fontList = {
    {"Minecraft", Enum.Font.SourceSansBold},
    {"Gotham", Enum.Font.GothamBold},
    {"Arial", Enum.Font.ArialBold},
    {"Roboto", Enum.Font.RobotoBold},
    {"Verdana", Enum.Font.Verdana},
    {"Comic Sans", Enum.Font.ComicSans},
    {"Impact", Enum.Font.Ubuntu},
    {"Montserrat", Enum.Font.Gotham},
    {"Poppins", Enum.Font.SourceSansLight},
    {"Open Sans", Enum.Font.OpenSans},
}

local function changeFont(font)
    pcall(function()
        -- Change font for all TextLabels and TextButtons in the game
        ...
        -- Change font for CoreGui elements too
    ...
    pcall(function()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.KeyCode == Enum.KeyCode.RightShift then
                ...
                if hub.Visible then
                    pcall(function()
                        hub:TweenSize(UDim2.new(0, 650, 0, 550), "Out", "Back", 0.5, true)
                    end)
                    toggleBtn.Text = "🖥️"
            ...
            task.spawn(function()
                while isHolding do
                    pcall(function()
                        ...
                        local lookVector = root.CFrame.LookVector
                        local newPos = root.Position + lookVector * 2
                        local tweenInfo = TweenInfo.new(0.05, Enum.EasingStyle.Linear)
                        local tween = TweenService:Create(root, tweenInfo, {CFrame = CFrame.new(newPos)})
                        tween:Play()
                        tween.Completed:Wait()
...
local stepsSlider = slider(p1, "STEP COUNT", 1, 200, 50, function(v) end)

section(p1, "FLOATING TELEPORT")
local floatToggle = toggle(p1, "Enable Floating Button", false, function(s)
    floatEnabled = s
    if floatEnabled then
        createFloatButton()
...
local floatStepSlider = slider(p1, "Float Step Size", 1, 50, 10, function(v)
    floatStep = math.floor(v)
end)

-- Direction buttons for floating
section(p1, "FLOAT DIRECTION")

local dirFrame = Instance.new("Frame")
dirFrame.Size = UDim2.new(1, 0, 0, 120)
dirFrame.BackgroundTransparency = 1
dirFrame.BorderSizePixel = 0
dirFrame.Parent = p1

local dirGrid = Instance.new("UIGridLayout")
dirGrid.CellSize = UDim2.new(0, 60, 0, 40)
dirGrid.CellPadding = UDim2.new(0, 10, 0, 5)
dirGrid.SortOrder = Enum.SortOrder.LayoutOrder
dirGrid.Parent = dirFrame

local dirButtons = {
    {"⬆", "Forward", function() if floatButton then floatButton.Text = "⬆" end end},
    {"⬇", "Backward", function() if floatButton then floatButton.Text = "⬇" end end},
    {"⬅", "Left", function() if floatButton then floatButton.Text = "⬅" end end},
    {"➡", "Right", function() if floatButton then floatButton.Text = "➡" end end},
    {"⬆⬆", "Up", function() if floatButton then floatButton.Text = "⬆⬆" end end},
    {"⬇⬇", "Down", function() if floatButton then floatButton.Text = "⬇⬇" end end},
}

for _, data in ipairs(dirButtons) do
    local b = Instance.new("TextButton")
    b.Size = UDim2.new(0, 60, 0, 40)
    b.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    b.BackgroundTransparency = 0.2
    b.BorderSizePixel = 1
    b.BorderColor3 = C.primary
    b.Text = data[1]
    ...
    bCorner.CornerRadius = UDim.new(0, 0)
    bCorner.Parent = b
    b.MouseButton1Click:Connect(data[3])
end
...
button(p1, "[ TELEPORT TO CHOSEN ]", function()
...
local espTransSlider = slider(p4, "Box Transparency", 0.2, 0.9, espBoxTransparency, function(v)
...
local tempSlider = slider(p5, "Color Temp", -1, 1, 0, function(v)
    visualsColorTemp = v
    applyVisuals()
end)
...
label(p7, "Click a button to change the skybox instantly!", C.gold)
...
local p8 = tabPages[8]
section(p8, "UNIVERSAL FONT CHANGER")
label(p8, "Click a font to change ALL text in the game!", C.gold)

for _, data in ipairs(fontList) do
    local btn = button(p8, "[ " .. data[1] .. " ]", function()
        changeFont(data[2])
    end, C.primary)
end

button(p8, "[ RESET FONT ]", function()
    changeFont(Enum.Font.SourceSansBold)
end, C.red)
...
ftText.Text = "$ REDDIEHUB v25.0 // KEY: Ali // TERMINAL EDITION"
...
print("📋 Features:")
print("   🖥️ Floating Teleport System")
print("   🌌 Skybox Changer - 3 Presets")
print("   🔤 Universal Font Changer - 10 Fonts")
print("   🎯 Camlock with Custom Image")
print("   🏃 Move Forward with Hold")
print("   📍 Choose/Unchoose Position")
print("   👁️ ESP - Auto-updates")
print("   💎 Black & White Terminal Style")
print("   🔑 Key: Ali")
print("==========================================")
